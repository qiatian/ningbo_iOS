...
FDFSStorageReservedSpace g_storage_reserved_space_s = {
...
TimeInfo g_trunk_create_file_time_base_s = {0, 0};
int g_trunk_create_file_interval_s = 86400;
int g_trunk_compress_binlog_min_interval_s = 0;
ConnectionInfo g_trunk_server = {-1, 0};
bool g_if_use_trunk_file_s = false;
bool g_if_trunker_self = false;
bool g_trunk_create_file_advance_s = false;
bool g_trunk_init_check_occupying_s = false;
bool g_trunk_init_reload_from_binlog_s = false;
static byte trunk_init_flag = STORAGE_TRUNK_INIT_FLAG_NONE;
int64_t g_trunk_total_free_space = 0;
int64_t g_trunk_create_file_space_threshold_s = 0;
...
int storage_trunk_init()
{
	...
	g_trunk_server.port = g_server_port_s;
...
static int storage_trunk_save()
{
	...
	if (!(g_trunk_compress_binlog_min_interval_s > 0 && \
...
{
	if (g_trunk_init_check_occupying_s)
	{
...
#define TRUNK_LINE_MAX_LENGHT  64
	...
	storage_trunk_get_data_filename(trunk_data_filename);
	if (g_trunk_init_reload_from_binlog_s)
	{
		if (unlink(trunk_data_filename) != 0)
		{
			result = errno != 0 ? errno : ENOENT;
...
int trunk_create_trunk_file_advance(void *args)
{
	...
	if (!g_trunk_create_file_advance_s)
	{
		logError("file: "__FILE__", line: %d, " \
	...
	alloc_space = g_trunk_create_file_space_threshold_s - \
			g_trunk_total_free_space;
	if (alloc_space <= 0)
	{
		logDebug("file: "__FILE__", line: %d, " \
	...
	if (!storage_check_reserved_space_path(total_mb_sum, free_mb_sum \
		- (alloc_space / FDFS_ONE_MB), g_storage_reserved_space_s.rs.mb))
	{
		logError("file: "__FILE__", line: %d, " \
