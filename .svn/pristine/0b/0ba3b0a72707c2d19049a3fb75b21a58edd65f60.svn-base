...
static int tracker_get_my_server_id()
{
	...
	if (g_use_storage_id_s)
	{
		...
		if (g_id_type_in_filename_s == FDFS_ID_TYPE_SERVER_ID)
		{
...
static int storage_check_and_make_data_dirs()
{
	...
	if (fileExists(full_filename))
	{
		...
		if (g_last_server_port == 0 || g_last_http_port == 0)
		{
			...
			{
				g_last_server_port = g_server_port_s;
			}
		...
		char *bind_addr, const int addr_size)
{
	...
	*/
	...
	do
	{
		...
		g_network_tv_s.tv_sec = g_fdfs_network_timeout;

		g_server_port_s = iniGetIntValue(NULL, "port", &iniContext, \
					FDFS_STORAGE_SERVER_DEF_PORT);
		if (g_server_port_s <= 0)
		{
			g_server_port_s = FDFS_STORAGE_SERVER_DEF_PORT;
		}
		...
		g_max_connections_s = iniGetIntValue(NULL, "max_connections", \
				&iniContext, DEFAULT_MAX_CONNECTONS);
		if (g_max_connections_s <= 0)
		{
			g_max_connections_s = DEFAULT_MAX_CONNECTONS;
		}
		if ((result=set_rlimit(RLIMIT_NOFILE, g_max_connections_s)) != 0)
		{
			break;
		}

		g_accept_threads_c = iniGetIntValue(NULL, "accept_threads", \
				&iniContext, 1);
		if (g_accept_threads_c <= 0)
		{
			logError("file: "__FILE__", line: %d, " \
				"item \"accept_threads\" is invalid, " \
				"value: %d <= 0!", __LINE__, g_accept_threads_c);
			result = EINVAL;
                        break;
		}

		g_work_threads_s = iniGetIntValue(NULL, "work_threads", \
				&iniContext, DEFAULT_WORK_THREADS);
		if (g_work_threads_s <= 0)
		{
			logError("file: "__FILE__", line: %d, " \
				"item \"work_threads\" is invalid, " \
				"value: %d <= 0!", __LINE__, g_work_threads_s);
			result = EINVAL;
                        break;
		}
		...
		*/
		...
		{
			snprintf(g_run_by_group_s, sizeof(g_run_by_group_s), \
				"%s", pRunByGroup);
		}
		if (*g_run_by_group_s == '\0')
		{
		...
		else
		{
			struct group *pGroup;
     			...
     			pGroup = getgrnam(g_run_by_group_s);
			if (pGroup == NULL)
			{
				result = errno != 0 ? errno : ENOENT;
				logError("file: "__FILE__", line: %d, " \
		...
		{
			snprintf(g_run_by_user_s, sizeof(g_run_by_user_s), \
				"%s", pRunByUser);
		}
		if (*g_run_by_user_s == '\0')
		{
		...
		else
		{
			struct passwd *pUser;
     			...
     			pUser = getpwnam(g_run_by_user_s);
			if (pUser == NULL)
			{
				result = errno != 0 ? errno : ENOENT;
				logError("file: "__FILE__", line: %d, " \
		...
		if ((result=load_allow_hosts(&iniContext, \
                	 &g_allow_ip_addrs_s, &g_allow_ip_count_s)) != 0)
		{
			return result;
		}
		...
		g_sync_log_buff_interval_s = iniGetIntValue(NULL, \
				"sync_log_buff_interval", &iniContext, \
				SYNC_LOG_BUFF_DEF_INTERVAL);
		if (g_sync_log_buff_interval_s <= 0)
		{
			g_sync_log_buff_interval_s = SYNC_LOG_BUFF_DEF_INTERVAL;
		}
		...
		g_thread_stack_size_s = (int)thread_stack_size;

		if (g_thread_stack_size_s < 64 * 1024)
		{
			logError("file: "__FILE__", line: %d, " \
				"item \"thread_stack_size\" %d is invalid, " \
				"which < %d", __LINE__, g_thread_stack_size_s, \
				64 * 1024);
			result = EINVAL;
			break;
		}
		...
		g_rotate_error_log_s = iniGetBoolValue(NULL, "rotate_error_log",\
					&iniContext, false);
		if ((result=get_time_item_from_conf(&iniContext, \
			"error_log_rotate_time", &g_error_log_rotate_time_s, \
			0, 0)) != 0)
		{
			break;
		}
		...
		logInfo("FastDFS v%d.%02d, base_path=%s, store_path_count=%d, " \
			...
			g_group_name, g_run_by_group_s, g_run_by_user_s, \
			g_fdfs_connect_timeout, g_fdfs_network_timeout, \
			g_server_port_s, bind_addr, \
			g_client_bind_addr, g_max_connections_s, \
			g_accept_threads_c, g_work_threads_s, g_disk_rw_separated, \
			...
			g_allow_ip_count_s, g_file_distribute_path_mode, \
			g_file_distribute_rotate_count, \
			g_fsync_after_written_bytes, g_sync_log_buff_interval_s, \
			g_sync_binlog_buff_interval, g_sync_stat_file_interval, \
			g_thread_stack_size_s/1024, g_upload_priority, \
			...
			g_rotate_error_log_s, g_error_log_rotate_time_s.hour, \
			g_error_log_rotate_time_s.minute, \
