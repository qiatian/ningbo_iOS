...
volatile bool g_continue_flag_s = true;
FDFSStorePathInfo *g_path_space_list = NULL;
int g_subdir_count_per_path = DEFAULT_DATA_DIR_COUNT_PER_PATH;

int g_server_port_s = FDFS_STORAGE_SERVER_DEF_PORT;
char g_http_domain[FDFS_DOMAIN_NAME_MAX_SIZE] = {0};
int g_http_port = 80;
int g_last_server_port = 0;
int g_last_http_port = 0;
int g_max_connections_s = DEFAULT_MAX_CONNECTONS;
int g_accept_threads_c = 1;
int g_work_threads_s = DEFAULT_WORK_THREADS;
...
struct timeval g_network_tv_s = {DEFAULT_NETWORK_TIMEOUT, 0};
...
int g_sync_log_buff_interval_s = SYNC_LOG_BUFF_DEF_INTERVAL;
...
bool g_rotate_error_log_s = false;  //if rotate the error log every day
bool g_use_storage_id_s = false;    //identify storage by ID instead of IP address
byte g_id_type_in_filename_s = FDFS_ID_TYPE_IP_ADDRESS; //id type of the storage server in the filename
bool g_store_slave_file_use_link_s = false; //if store slave file use symbol link
...
int g_allow_ip_count_s = 0;
in_addr_t *g_allow_ip_addrs_s = NULL;

TimeInfo g_access_log_rotate_time = {0, 0}; //rotate access log time base
TimeInfo g_error_log_rotate_time_s  = {0, 0}; //rotate error log time base

gid_t g_run_by_gid;
uid_t g_run_by_uid;

char g_run_by_group_s[32] = {0};
char g_run_by_user_s[32] = {0};

char g_bind_addr[IP_ADDRESS_SIZE] = {0};
bool g_client_bind_addr = true;
bool g_storage_ip_changed_auto_adjust_s = false;
bool g_thread_kill_done = false;
bool g_file_sync_skip_invalid_record = false;

int g_thread_stack_size_s = 512 * 1024;
int g_upload_priority = DEFAULT_UPLOAD_PRIORITY;
time_t g_up_time_s = 0;
