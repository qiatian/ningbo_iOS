...
static void *tracker_report_thread_entrance(void *arg)
{
	...
	bServerPortChanged = (g_last_server_port != 0) && \
				(g_server_port_s != g_last_server_port);
		...
		*/
		...
		while (g_continue_flag_s)
		{
					...
					g_heart_beat_interval)
			{
				...
				if (g_storage_ip_changed_auto_adjust_s && \
		...
		if ((!g_continue_flag_s) && fdfs_quit(pTrackerServer) != 0)
		{
		}
	...
	bool *bServerPortChanged)
{
		...
		*(g_trunk_server.ip_addr + (IP_ADDRESS_SIZE - 1)) = '\0';
			...
			g_trunk_server.port == g_server_port_s)
		{
			...
			if (g_trunk_create_file_advance_s && \
				g_trunk_create_file_interval_s > 0)
			{
			ScheduleArray scheduleArray;
			ScheduleEntry entries[1];

			entries[0].id = TRUNK_FILE_CREATOR_TASK_ID;
			entries[0].time_base = g_trunk_create_file_time_base_s;
			entries[0].interval = g_trunk_create_file_interval_s;
	...
	*/
	...
	if (*bServerPortChanged)
	{
		...
		if (g_server_port_s != g_last_server_port)
		{
			...
			const int tracker_index, const bool sync_old_done)
{
	...
	long2buff(g_server_port_s, pReqBody->storage_port);
	...
	long2buff(g_up_time_s, pReqBody->up_time);
		...
		do
		{
			...
			else if (server_status == FDFS_STORAGE_STATUS_IP_CHANGED)
			{
				if (!g_use_storage_id_s)
				{
					...
					g_server_port_s, pNewStorageId, g_server_port_s);
					if (strcmp(g_sync_src_id, pOldStorageId) == 0)
					{
...
int tracker_report_thread_start()
{
	...
	if ((result=init_pthread_attr(&pattr, g_thread_stack_size_s)) != 0)
	{
		return result;
	}
