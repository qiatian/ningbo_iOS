...
int storage_service_init()
{
	...
	if ((result=init_pthread_attr(&thread_attr, g_thread_stack_size_s)) != 0)
	{
		logError("file: "__FILE__", line: %d, " \
	...
	if ((result=free_queue_init(g_max_connections_s, g_buff_size, \
                g_buff_size, sizeof(StorageClientInfo))) != 0)
	{
		return result;
	}

	bytes = sizeof(struct storage_nio_thread_data) * g_work_threads_s;
...
static void *accept_thread_entrance(void* arg)
{
	...
	while (g_continue_flag_s)
	{
		...
		if (g_allow_ip_count_s >= 0)
		{
			if (bsearch(&client_addr, g_allow_ip_addrs_s, \
					g_allow_ip_count_s, sizeof(in_addr_t), \
		...
		pClientInfo->nio_thread_index = pTask->event.fd % g_work_threads_s;
...
void storage_accept_loop(int server_sock)
{
	if (g_accept_threads_c > 1)
	{
		...
		{
			for (i=1; i<g_accept_threads_c; i++)
			{
			if ((result=pthread_create(&tid, &thread_attr, \
...
static void *work_thread_entrance(void* arg)
{
	...
	ioevent_loop(&pThreadData->thread_data, storage_recv_notify_read,
		task_finish_clean_up_s, &g_continue_flag_s);
...
static int storage_service_upload_file_done(struct fast_task_info *pTask)
{
	...
	*new_filename = '\0';
	...
	if ((pFileContext->extra_info.upload.file_type & _FILE_TYPE_SLAVE))
	{
		...
		if (g_store_slave_file_use_link_s)
		{
...
static int storage_upload_file(struct fast_task_info *pTask, bool bAppenderFile)
{
	...
	*(file_ext_name + FDFS_FILE_EXT_NAME_MAX_LEN) = '\0';
	...
	{
		if (g_if_use_trunk_file_s && trunk_check_size( \
			...
			g_avg_storage_reserved_mb))
		{
			logError("file: "__FILE__", line: %d, " \
				...
				fdfs_storage_reserved_space_to_string_ex( \
				  g_storage_reserved_space_s.flag, \
          g_avg_storage_reserved_mb, \
				  g_path_space_list[store_path_index]. \
				  total_mb, g_storage_reserved_space_s.rs.ratio,\
				  reserved_space_str));
			return ENOSPC;
		}
		...
		g_avg_storage_reserved_mb))
	{
		logError("file: "__FILE__", line: %d, " \
			...
			file_bytes, fdfs_storage_reserved_space_to_string_ex(\
				...
				g_storage_reserved_space_s.rs.ratio, \
				reserved_space_str));
		return ENOSPC;
	}
