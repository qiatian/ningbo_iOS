...
int storage_get_params_from_tracker()
{
	...
	if ((result=fdfs_get_ini_context_from_tracker(&g_tracker_group, \
		&iniContext, (bool * volatile)&g_continue_flag_s, \
		g_client_bind_addr, g_bind_addr)) != 0)
	{
		return result;
	}

	g_storage_ip_changed_auto_adjust_s = iniGetBoolValue(NULL, \
	...
	if ((result=fdfs_parse_storage_reserved_space(&iniContext, \
		&g_storage_reserved_space_s)) != 0)
	{
		iniFreeContext(&iniContext);
		return result;
	}
	if (g_storage_reserved_space_s.flag == \
		TRACKER_STORAGE_RESERVED_SPACE_FLAG_MB)
	{
		g_avg_storage_reserved_mb = g_storage_reserved_space_s.rs.mb \
	...
	g_use_storage_id_s = iniGetBoolValue(NULL, "use_storage_id", \
				&iniContext, false);
	...
	g_trunk_create_file_advance_s = iniGetBoolValue(NULL, \
			"trunk_create_file_advance", &iniContext, false);
	if ((result=get_time_item_from_conf(&iniContext, \
               	"trunk_create_file_time_base", \
		&g_trunk_create_file_time_base_s, 2, 0)) != 0)
	{
		iniFreeContext(&iniContext);
		return result;
	}
	g_trunk_create_file_interval_s = iniGetIntValue(NULL, \
			"trunk_create_file_interval", &iniContext, \
			86400);
	g_trunk_create_file_space_threshold_s = iniGetInt64Value(NULL, \
			"trunk_create_file_space_threshold", \
			&iniContext, 0);

	g_trunk_init_check_occupying_s = iniGetBoolValue(NULL, \
			"trunk_init_check_occupying", &iniContext, false);
	g_trunk_init_reload_from_binlog_s = iniGetBoolValue(NULL, \
			"trunk_init_reload_from_binlog", &iniContext, false);
	g_trunk_compress_binlog_min_interval_s = iniGetIntValue(NULL, \
			"trunk_compress_binlog_min_interval", &iniContext, 0);

	g_store_slave_file_use_link_s = iniGetBoolValue(NULL, \
	...
	if (pIdType != NULL && strcasecmp(pIdType, "id") == 0)
	{
		...
		{
			g_id_type_in_filename_s = FDFS_ID_TYPE_SERVER_ID;
		}
		else
		{
			g_id_type_in_filename_s = FDFS_ID_TYPE_IP_ADDRESS;
		}
	}
	else 
	{
	...
	if (use_trunk_file && !g_if_use_trunk_file_s)
	{
	...
	g_if_use_trunk_file_s = use_trunk_file;
	...
	logInfo("file: "__FILE__", line: %d, " \
		...
		__LINE__, g_use_storage_id_s, \
		g_id_type_in_filename_s == FDFS_ID_TYPE_SERVER_ID ? "id" : "ip", \
		g_storage_ip_changed_auto_adjust_s, \
		g_store_path_mode, fdfs_storage_reserved_space_to_string( \
			&g_storage_reserved_space_s, reserved_space_str), \
		g_if_use_trunk_file_s, g_slot_min_size, \
		g_slot_max_size / FDFS_ONE_MB, \
		g_trunk_file_size / FDFS_ONE_MB, \
		g_trunk_create_file_advance_s, \
		g_trunk_create_file_time_base_s.hour, \
		g_trunk_create_file_time_base_s.minute, \
		g_trunk_create_file_interval_s, \
		(int)(g_trunk_create_file_space_threshold_s / \
		(FDFS_ONE_MB * 1024)), g_trunk_init_check_occupying_s, \
		g_trunk_init_reload_from_binlog_s, \
		g_trunk_compress_binlog_min_interval_s, \
		g_store_slave_file_use_link_s);

	if (g_use_storage_id_s && *g_sync_src_id != '\0' && \
