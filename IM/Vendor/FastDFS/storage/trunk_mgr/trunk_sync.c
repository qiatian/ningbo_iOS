...
{
	return trunk_get_mark_filename_by_id_and_port(storage_id, g_server_port_s, \
...
static void* trunk_sync_thread_entrance(void* arg)
{
	...
	while (g_continue_flag_s && g_if_trunker_self && \
		...
		pStorage->status != FDFS_STORAGE_STATUS_NONE)
	{
		...
		if ((!g_continue_flag_s) || (!g_if_trunker_self) || \
			pStorage->status == FDFS_STORAGE_STATUS_DELETED || \
			pStorage->status == FDFS_STORAGE_STATUS_IP_CHANGED || \
			pStorage->status == FDFS_STORAGE_STATUS_NONE)
		{
			logError("file: "__FILE__", line: %d, break loop." \
				...
				g_continue_flag_s, g_if_trunker_self, \
				pStorage->status);
			break;
		}
