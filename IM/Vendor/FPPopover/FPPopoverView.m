...
-(void)setupViews
{
    ...
    if(_arrowDirection == FPPopoverArrowDirectionUp)
    {
        contentRect.origin = CGPointMake(FP_POPOVER_ORGIN_X, 60);
        contentRect.size = CGSizeMake(self.bounds.size.width-2*FP_POPOVER_ORGIN_X, self.bounds.size.height-70);
        _titleLabel.frame = CGRectMake(FP_POPOVER_ORGIN_X, 30, self.bounds.size.width-2*FP_POPOVER_ORGIN_X, 20);
		if (self.title==nil || self.title.length==0) {
			contentRect.origin = CGPointMake(FP_POPOVER_ORGIN_X, 20);
			contentRect.size = CGSizeMake(self.bounds.size.width-2*FP_POPOVER_ORGIN_X, self.bounds.size.height-20);
		}
    }
    else if(_arrowDirection == FPPopoverArrowDirectionDown)
    {
        contentRect.origin = CGPointMake(FP_POPOVER_ORGIN_X, 40);
        contentRect.size = CGSizeMake(self.bounds.size.width-2*FP_POPOVER_ORGIN_X, self.bounds.size.height-70);
        _titleLabel.frame = CGRectMake(FP_POPOVER_ORGIN_X, 10, self.bounds.size.width-2*FP_POPOVER_ORGIN_X, 20);
		if (self.title==nil || self.title.length==0) {
			contentRect.origin = CGPointMake(FP_POPOVER_ORGIN_X, 10);
			contentRect.size = CGSizeMake(self.bounds.size.width-2*FP_POPOVER_ORGIN_X, self.bounds.size.height-40);
		}
    }
